...
from remediation_tools import (
    detect_remediation_request,
    format_remediation_plan,
    build_pre_remediation_diagnosis,
)
...
def classify_incident_node(state: AgentState) -> AgentState:
    ...
    remediation_requested = detect_remediation_request(question)
...
def devops_agent_node(state: AgentState) -> AgentState:
    if state.get("remediation_requested"):
        answer = build_pre_remediation_diagnosis(
...
def remediation_plan_node(state: AgentState) -> AgentState:
    if not state.get("remediation_requested"):
        return state

    remediation = format_remediation_plan(
        question=state["question"],
...
def build_graph():
    ...
    graph.add_node("remediation_plan", remediation_plan_node)
    ...
    graph.add_edge("devops_agent", "remediation_plan")
    graph.add_edge("remediation_plan", "chatops_report")
